...
// GetPackageObject gets the ChaincodeDeploymentSpec as protos.Message
...
func (ccpack *SignedCDSPackage) InitFromBuffer(buf []byte) (*ChaincodeData, error) {
	...
	cHdr, sDepSpec, err := ExtractSignedCCDepSpec(env)
	if err != nil {
		return nil, err
	}

	if cHdr.Type != int32(cb.HeaderType_CHAINCODE_PACKAGE) {
