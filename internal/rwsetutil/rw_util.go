...
// TxPvtRwSet represents 'rwset.TxPvtReadWriteSet' protos message
type TxPvtRwSet struct {
	NsPvtRwSet []*NsPvtRwSet
}

// NsPvtRwSet represents 'rwset.NsPvtReadWriteSet' protos message
type NsPvtRwSet struct {
	NameSpace     string
	CollPvtRwSets []*CollPvtRwSet
}

// CollPvtRwSet encapsulates 'kvrwset.KVRWSet' protos message for a private rwset for a specific collection
...
/////////////////////////////////////////////////////////////////
// FUNCTIONS for converting messages to/from protos bytes
/////////////////////////////////////////////////////////////////

// ToProtoBytes constructs TxReadWriteSet protos message and serializes using protobuf Marshal
...
// FromProtoBytes deserializes protobytes into TxReadWriteSet protos message and populates 'TxRwSet'
...
// ToProtoBytes constructs 'TxPvtReadWriteSet' protos message and serializes using protobuf Marshal
...
// FromProtoBytes deserializes protobytes into 'TxPvtReadWriteSet' protos message and populates 'TxPvtRwSet'
...
// NewKVRead helps constructing protos message kvrwset.KVRead
func NewKVRead(key string, version *Height) *kvrwset.KVRead {
	return &kvrwset.KVRead{Key: key, Version: newProtoVersion(version)}
}

// NewVersion helps converting protos message kvrwset.Version to version.Height
func NewVersion(protoVersion *kvrwset.Version) *Height {
	if protoVersion == nil {
		return nil
	}
	return NewHeight(protoVersion.BlockNum, protoVersion.TxNum)
}

func newProtoVersion(height *Height) *kvrwset.Version {
	if height == nil {
		return nil
	}
...
func newPvtKVReadHash(key string, version *Height) *kvrwset.KVReadHash {
	return &kvrwset.KVReadHash{KeyHash: utils.ComputeStringHash(key), Version: newProtoVersion(version)}
}
...
func newPvtKVWriteAndHash(key string, value []byte) (*kvrwset.KVWrite, *kvrwset.KVWriteHash) {
	...
	keyHash = utils.ComputeStringHash(key)
	if !kvWrite.IsDelete {
		valueHash = utils.ComputeHash(value)
	}
	return kvWrite, &kvrwset.KVWriteHash{KeyHash: keyHash, IsDelete: kvWrite.IsDelete, ValueHash: valueHash}
}

func ProtoUnmarshal(data []byte) (*TxRwSet, error) {
	txrwSet := &TxRwSet{}
	err := txrwSet.FromProtoBytes(data)
	if err != nil {
		return nil, err
	}
	return txrwSet, nil
